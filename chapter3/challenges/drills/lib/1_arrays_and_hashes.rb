...
def starts_with_a_vowel(array)
    array.select {|string| string.start_with?(/[aeiou]/)}
end
...
def greater_than(array, number)
    array.select {|value| value > number}
end
...
def words_shorter_than(array, number)
    array.select {|value| value.length<number}
end
...
def where_key_starts_with(hash, letter)
    hash.select {|word| word.start_with?(letter)}
end
...
def sort_by_value(hash)
    hash.sort_by {|key, value| value}.to_h #sorts by value but sort_by returns array, has to be converted back to_h
end
